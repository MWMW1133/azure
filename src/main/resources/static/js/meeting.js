(function (w) {
    ...
    /* ===========================
     * A) 설정/백엔드/Agora (UI 변경 없음)
     * =========================== */
    const Cfg = (() => {
        const q = (k) => new URLSearchParams(location.search).get(k);
        const A = w.APP || {};
        return {
            appId: String(A.agoraAppId || ''),
            eventId: Number(A.eventId || q('eventId')),
            meetingId: Number(A.meetingId || q('meetingId')),
            uid: String(A.uid || q('uid') || Math.floor(100000 + Math.random()*900000)),
            csrf: A.csrf || ''
        };
    })();
    ...
    const MeetingApi = (() => {
        const headers = () => {
            ...
            if (Cfg.csrf) h['X-CSRF-TOKEN'] = Cfg.csrf;
            return h;
        };
        const post = (url, data) =>
        ...
        return {
            token: (channel, uid) => post('/api/rtc/token', { channel, uid }),
            startRec: (eventId, uid) => post('/api/recordings/start', { eventId, uid }),
            stopRec: (meetingId, channel, uid, resourceId, sid) =>
                post('/api/recordings/stop', { meetingId, channel, uid, resourceId, sid })
        };
    })();

    const Comm = (() => {
        let client=null, localTrack=null, channel=null, resourceId=null, sid=null;

        async function join(){
            if (!w.AgoraRTC) throw new Error('Agora SDK not loaded');
            channel = 'event-' + Cfg.eventId;
            const { token } = await MeetingApi.token(channel, Cfg.uid);
            client = AgoraRTC.createClient({ mode:'rtc', codec:'vp8' });
            await client.join(Cfg.appId, channel, token, Cfg.uid);
            localTrack = await AgoraRTC.createMicrophoneAudioTrack();
            await client.publish([localTrack]);
        }
        async function leave(){
            try { if(localTrack) localTrack.close(); if(client) await client.leave(); }
            finally { client=null; localTrack=null; }
        }
        async function startRecording(){
            const r = await MeetingApi.startRec(Cfg.eventId, Cfg.uid);
            resourceId = r.resourceId; sid = r.sid; if (r.channel) channel = r.channel;
        }
        async function stopRecording(){
            if (!resourceId || !sid) return;
            await MeetingApi.stopRec(Cfg.meetingId, channel, Cfg.uid, resourceId, sid);
            resourceId=null; sid=null;
        }
        async function switchMic(deviceId){
            if (!localTrack || !deviceId) return;
            try { await localTrack.setDevice(deviceId); }
            catch {
                const newTrack = await AgoraRTC.createMicrophoneAudioTrack({ microphoneId: deviceId });
                await client.unpublish([localTrack]); localTrack.close();
                localTrack = newTrack; await client.publish([localTrack]);
            }
        }
        return { join, leave, startRecording, stopRecording, switchMic };
    })();
...
<section class="room-wrap">
  ...
  <div id="notes-modal" class="notes-modal" aria-hidden="true">
    ...
    function bindRoomEvents(root){
        ...
        // 작성 시작/종료 (+ Cloud Recording)
        async function beginNotes(){
            startBtn.disabled = true; stopBtn.disabled = false;
            openBtn.classList.remove('show');
            Speech.start();
            root.classList.add('rec-on');
            try { await Comm.startRecording(); } catch(e){ console.error(e); alert('녹음 시작 실패'); }
        }
        async function endNotes(){
            startBtn.disabled = false; stopBtn.disabled = true;
            Speech.stop();
            root.classList.remove('rec-on');
            try { await Comm.stopRecording(); } catch(e){ console.error(e); alert('녹음 종료 실패'); }
        ...
        // 통화 종료 (녹음 중이면 먼저 종료) → 토스트는 기존 그대로
        endBtn?.addEventListener('click', async () => {
            if (!startBtn.disabled) {} else { await endNotes(); }
            try { await Comm.leave(); } catch(e){ console.warn(e); }
            ...
            if (navigator.mediaDevices?.enumerateDevices) {
                navigator.mediaDevices.enumerateDevices().then(list=>{
                    ...
                    if (mics.length && micSel){
                        ...
                        micSel.addEventListener('change', e => Comm.switchMic(e.target.value));
                    }
                });
            }
        }catch(e){}
