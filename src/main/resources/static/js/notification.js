...
<<<<<<< HEAD
  // ✅ 스코프 한정 + 올바른 선택자
  const notifList = document.querySelector(".topbar #notifList");
  if (!notifList) return;
  ...
  //  렌더링 함수  (✔ .notif-item 클래스를 앵커에 추가)
  const renderNotifs = (data) => {
    notifList.innerHTML = "";
    data.forEach((n) => {
      notifList.innerHTML += `
        <a href="${n.link}"
           class="notif-item text-decoration-none text-dark"
           data-id="${n.id}">
          <div class="notif-card ${n.isRead ? 'opacity-75' : ''}">
            <div class="fw-bold">${n.title}</div>
            <div>${n.message}</div>
...
<<<<<<< HEAD
    ...
    if (notif && !notif.isRead) {
      ...
      // TODO: 서버 연동 (읽음 처리)
      // fetch(`/api/notifications/${id}/read`, { method: "POST" })
      //   .then(res => { if (res.ok) console.log("알림 읽음 처리 완료"); })
      //   .catch(err => console.error("에러 발생", err));
    }
  });
