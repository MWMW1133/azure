...
public interface DocumentVersionRepository extends JpaRepository<DocumentVersion, Long> {
    ...
    // file_objects 파일 지우려면 여기서 지워야함
    void deleteByFile_Id(Long fileId);

    Long file(FileObject file);

    List<DocumentVersion> findByFile_Id(Long documentId);


    @Query("SELECT dv.document.id FROM DocumentVersion dv WHERE dv.file.id = :fileId")
    List<Long> findDocumentIdsByFileId(@Param("fileId") Long fileId);
